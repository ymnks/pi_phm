...
class PIPHM(nn.Module):
    """PI-PHM: Physics-Informed PatchTST-Mamba Hybrid Model"""
    ...
    def __init__(self, config: 'PI_PHM_Config', feature_index_map: Dict[str, Union[int, List[int]]], input_channels: int = 103):
        ...
        Args:
            ...
            input_channels: 输入通道数，默认为103（根据特征工程结果，包含KH0217）
        """
        super().__init__()
        self.config = config
    ...
    @classmethod
    def from_config(cls, config: 'PI_PHM_Config', feature_index_map: Dict[str, Union[int, List[int]]], input_channels: int = 103):
        """从配置创建模型的类方法"""
    ...
    def forward(self, x_dynamic: torch.Tensor, x_static: torch.Tensor, mask: torch.Tensor = None) -> Dict[str, torch.Tensor]:
        ...
        # 3. Mamba 全局建模
        h_mamba, ssm_states = self.mamba(h_patches)  # (B, 11, 128)
        
        # 4. 物理门控调制
        h_gated, gate_info = self.physics_gate(h_mamba, x_dynamic)  # (B, 11, 128)
...
if __name__ == "__main__":
    ...
    assert outputs["attn_weights"].shape == (B, config.model.n_heads, 11, 11)
    assert outputs["pool_attention"].shape == (B, 11)
    assert outputs["gate_info"]["physics_summary"].shape == (B, 8)
