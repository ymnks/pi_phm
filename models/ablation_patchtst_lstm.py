...
class PIPHM_LSTM(nn.Module):
    """
    Physics-Informed Patch Hybrid Model with LSTM
    使用LSTM替代Mamba进行全局序列建模
    """
    ...
    def forward(self, x_dynamic: torch.Tensor, x_static: torch.Tensor, mask: torch.Tensor = None) -> Dict[str, torch.Tensor]:
        ...
        # 3. LSTM 全局建模
        h_lstm, lstm_hidden = self.lstm(h_patches)  # (B, 11, 128)
        
        # 4. 物理门控调制
        h_gated, gate_info = self.physics_gate(h_lstm, x_dynamic)  # (B, 11, 128)
        ...
        outputs = {
            ...
            "gate_info": gate_info,
            "lstm_hidden": lstm_hidden,  # 替代ssm_states
        }
        return outputs
...
if __name__ == "__main__":
    ...
    # 测试完整模型
    model = PIPHM_LSTM.from_config(config, feature_index_map)
    outputs = model(x_dynamic, x_static, mask)
    
    print("PIPHM_LSTM Test Results:")
